...
app.get('/profile/:username', (req, res) => {
  ...
  db.get('SELECT id, username, created_at FROM users WHERE username = ?', [username], (err, profileUser) => {
    if (err) return res.status(500).send(err.message);
    if (!profileUser) return res.status(404).send('User not found');
    
    db.all('SELECT * FROM posts WHERE user_id = ? ORDER BY created_at DESC', [profileUser.id], (err, posts) => {
      if (err) return res.status(500).send(err.message);
      
      posts = posts.map(post => ({ ...post, username: profileUser.username }));
